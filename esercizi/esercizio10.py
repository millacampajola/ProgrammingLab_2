...
# classe che ritorna l'incremento medio e la predizione
class IncrementModel(Model):

    def __str__(self):
        return 'IncrementModel'
        
    # funzione per calcolare l'incremento medio
    def compute_avg_increment(self, data):
        ...
        for item in data:
            ...
            # Calcolo l'incremento ma non se sono al primo
            # giro ovvero se non è definito il "prev_item"
            if prev_item is not None:
                increments = increments + (item - prev_item)

            # Assegno questo valore come precedente
            prev_item = item

        # Calcolo l'incremento medio divivendo la somma degli incrementi sul totale dei dati (meno uno)
...
class FitIncrementModel(IncrementModel):
    ...
    def predict(self, predict_data):
        ...
        # così da avere l'incremento "originale"
...
# ma nella realtà avrei usato l'oggetto CSVFile e caricato i dati dal file. Ma così evito di
# avere troppe cose su cui sto lavorando assieme, e visto che i dati sono piccoli, posso farlo
# ed è comodo (se avevo migliaia di valori forse era meglio di no).
# i valori delle vendite di shampoo per 36 mesi 
